...
if __name__ == "__main__":
    ...
    model_path = "./model-seq_c-y_sample_c1.pth"  
    
    

    Load_model_continue_path="./model-seq_c-y_sample.pth"
    
    Load_model_path="./model_x-c_small_sample.pth"
    
    # Set device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"device={device}")
    pkl_dir="./class_attr_data_10/"
    batch_size, num_workers = 5,0
    ...
    model.load_state_dict(torch.load(Load_model_continue_path,weights_only=True))
    ...
    for epoch in range(num_epochs):
        ...
        accuracy = correct / total
        print(f"Epoch [{epoch+1}/{num_epochs}], test-accu_x-y: {(accuracy):.4f}") 
